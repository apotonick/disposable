...
class TwinCompositionTest < MiniTest::Spec
  ...
  it do
    ...
    _(request.song_id).must_equal 2
    _(request.name).must_equal 'Greg Howe'
    ...
    _(request.name).must_equal 'Wooten'

    # does not write to model.
    _(song.title).must_equal 'Extraction'
    _(requester.name).must_equal 'Greg Howe'
    ...
    _(requester.id).must_equal 1
    _(requester.name).must_equal 'Wooten'
  ...
  it do
    ...
    _(nested_hash).must_equal(song: { 'title' => 'Tease', 'id' => 2 }, requester: { 'name' => 'Wooten', 'id' => 1, 'captcha' => 'Awesome!' })
  end
  ...
  it do
    ...
    _(request.save).must_equal false
  end
end
