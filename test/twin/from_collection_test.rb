...
class TwinFromCollectionDecoratorTest < MiniTest::Spec
  ...
  describe 'from a collection' do
    it do
      ...
      _(twined_collection[0]).must_be_instance_of Twin::Artist
      _(twined_collection[0].model).must_equal artist1
      _(twined_collection[1]).must_be_instance_of Twin::Artist
      _(twined_collection[1].model).must_equal artist2
    end
  end
end
