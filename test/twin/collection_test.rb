...
class TwinCollectionTest < MiniTest::Spec
  ...
  describe 'reader for collection' do
    it do
      ...
      _(twin.songs.size).must_equal 1
      _(twin.songs[0].title).must_equal 'Broken'
  ...
  describe '#find_by' do
    ...
    it { _(twin.songs.find_by(id: 1)).must_be_nil }
    it { _(twin.songs.find_by(id: 3)).must_equal twin.songs[0] }
    it { _(twin.songs.find_by(id: 4)).must_equal twin.songs[1] }
    it { _(twin.songs.find_by(id: '4')).must_equal twin.songs[1] }
  end
end
...
class TwinCollectionActiveRecordTest < MiniTest::Spec
  ...
  it do
    ...
    _(twin.songs[0]).must_be_instance_of Twin::Song # twin wraps << added in twin.
    _(twin.songs[1]).must_be_instance_of Twin::Song

    # twin.songs[0].persisted?.must_equal false
    _(twin.songs[0].send(:model).persisted?).must_equal false
    _(twin.songs[1].send(:model).persisted?).must_equal true

    _(album.songs.size).must_equal 0 # nothing synced, yet.
    ...
    _(album.name).must_equal 'The Rest Is Silence'

    _(album.songs.size).must_equal 2 # synced!

    _(album.songs[0].persisted?).must_equal true
    _(album.songs[1].persisted?).must_equal true
    _(album.songs[0].title).must_equal 'Snorty Pacifical Rascal'
    _(album.songs[1].title).must_equal 'At Any Cost'
  end
  ...
  describe '#delete' do
    ...
    it do
      ...
      _(album.songs.size).must_equal 1 # not synced, yet.

      twin.save

      _(twin.songs.size).must_equal 0
      _(album.songs.size).must_equal 0
      _(song1.persisted?).must_equal true
    end
  ...
  describe '#destroy' do
    ...
    it do
      ...
      _(twin.songs.size).must_equal 0
      _(album.songs.size).must_equal 0
      _(song1.persisted?).must_equal false
    end
  end
  ...
  describe '#added' do
    ...
    it do
      ...
      twin.songs << song2
      _(twin.songs.added).must_equal [twin.songs[1]]
      twin.songs.insert(2, Song.new)
      _(twin.songs.added).must_equal [twin.songs[1], twin.songs[2]]
  ...
  describe '#deleted' do
    ...
    it do
      ...
      _(twin.songs).must_equal [twin.songs[0]]

      _(twin.songs.deleted).must_equal [deleted1, deleted2]
    end
...
class CollectionUnitTest < MiniTest::Spec
  ...
  # #insert(index, model)
  it do
    _(collection.insert(0, Model::Album.new)).must_be_instance_of Twin::Album
  end

  # #append(model)
  it do
    _(collection.append(Model::Album.new)).must_be_instance_of Twin::Album
    _(collection[0]).must_be_instance_of Twin::Album

    # allows subsequent calls.
    collection.append(Model::Album.new)
    _(collection[1]).must_be_instance_of Twin::Album
  ...
  # #<<
  it do
    _((collection << Model::Album.new)).must_be_instance_of Array
    _(collection[0]).must_be_instance_of Twin::Album
  end
end
