...
class InheritTest < MiniTest::Spec
  ...
  it do
    _(Twin::Album.definitions.get(:name).extend(Declarative::Inspect).inspect).must_equal '#<Disposable::Twin::Definition: @options={:fromage=>:_name, :private_name=>:name, :name=>"name"}>'
    _(Twin::Compilation.definitions.get(:name).extend(Declarative::Inspect).inspect).must_equal '#<Disposable::Twin::Definition: @options={:fromage=>:_name, :private_name=>:name, :name=>"name", :writeable=>false}>' # FIXME: where did :inherit go?
  end

  let(:album) { Model::Album.new('In The Meantime And Inbetween Time', [], Model::Artist.new) }

  it { _(Twin::Album.new(album).artist.artist_id).must_equal 1 }

  # inherit inline twins when not overriding.
  it { _(Twin::EmptyCompilation.new(album).artist.artist_id).must_equal 1 }

  # inherit inline twins when overriding.
  it { _(Twin::Compilation.new(album).artist.artist_id).must_equal 1 }
  ...
  describe 'custom accessors get inherited' do
    ...
    it do
      ...
      _(artist.name).must_equal('neewolleh')
